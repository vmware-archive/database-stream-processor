...
ROOT="${THIS_DIR}/../"
SERVER_DIR="${ROOT}/pipeline_server/"
...
psql -h localhost -U postgres -f "${ROOT}/pipeline_server/create_db.sql"
...
do
    ...
    project_id=$("${SERVER_DIR}/new_project.sh" "${project_name}" < "${project_dir}/project.sql" | jq '.project_id')
    ...
    do
        ...
        config_id=$("${SERVER_DIR}/new_config.sh" "${project_id}" "${config_name}"  < "${config_file}" | jq '.config_id')
