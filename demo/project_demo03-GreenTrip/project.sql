...
CREATE VIEW FEATURES as SELECT
    *,
    COUNT(*) OVER(
      ...
      RANGE BETWEEN 3600  PRECEDING AND 0 PRECEDING ) AS count_trips_window_1h_pickup_zip,
    AVG(fare_amount) OVER(
      PARTITION BY  pickup_location_id
      ORDER BY  extract (EPOCH from  CAST (lpep_pickup_datetime AS TIMESTAMP) )
      -- 1 hour is 3600  seconds
      RANGE BETWEEN 3600  PRECEDING AND 0 PRECEDING ) AS mean_fare_window_1h_pickup_zip,
    COUNT(*) OVER(
       ...
       RANGE BETWEEN 1800  PRECEDING AND 0 PRECEDING ) AS count_trips_window_30m_dropoff_zip,
