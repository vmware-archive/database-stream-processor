    ...
    <O as TryInto<usize>>::Error: Debug,
{
    ...
    type ChildKey = L::Key;
    type Item = (K, L::Item);
    type Cursor = OrderedCursor<K, L, O>;
    ...
    <O as TryInto<usize>>::Error: Debug,
{
    fn with_capacity(keys: usize, tuples: usize) -> Self {
        ...
        OrderedBuilder {
            ...
            offs,
            vals: L::with_capacity(tuples, tuples),
        }
    }
    #[inline]
...
#[derive(Debug)]
pub struct OrderedCursor<K, L: Trie, O> {
    // keys: OwningRef<Rc<Erased>, [K]>,
    // offs: OwningRef<Rc<Erased>, [usize]>,
    pos: usize,
    bounds: (usize, usize),
    _types: PhantomData<(K, L, O)>,
}

impl<K, L, O> Cursor for OrderedCursor<K, L, O>
where
    K: Ord,
    L: Trie,
    O: OrdOffset,
    ...
    <O as TryInto<usize>>::Error: Debug,
{
    type Key = K;
    type ChildKey = L::Key;
    type Storage = OrderedLayer<K, L, O>;
    type ValueStorage = L;
    ...
    fn rewind(&mut self, _storage: &OrderedLayer<K, L, O>) {
        self.pos = self.bounds.0;
    }
    fn reposition(&mut self, _storage: &OrderedLayer<K, L, O>, lower: usize, upper: usize) {
        self.pos = lower;
